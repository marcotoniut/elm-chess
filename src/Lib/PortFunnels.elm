...
port module Lib.PortFunnels exposing
  ( FunnelDict
  , Handler(..)
  , State
  , getCmdPort
...
-}
type Funnel m msg
  = WebSocketFunnel (FunnelSpec State WebSocket.State WebSocket.Message WebSocket.Response m msg)
...
-}
type Handler m msg
  = WebSocketHandler (WebSocket.Response -> State -> m -> (m, Cmd msg))
...
-}
handlerToFunnel : Handler m msg -> (String, Funnel m msg)
...
-}
appTrampoline : (String -> m -> (Value -> Cmd msg)) -> GenericMessage -> Funnel m msg -> State -> m -> Result String (m, Cmd msg)
...
-}
subscriptions : (Value -> msg) -> m -> Sub msg
subscriptions process model = subPort process
...
getCmdPort tagger moduleName useSimulator =
    if not useSimulator
    then cmdPort
    else case Dict.get moduleName simulatedPortDict of
...
-}
type alias FunnelDict m msg =
  (Dict String (Funnel m msg), String -> m -> (Value -> Cmd msg))


{-| Make a `Dict` mapping `moduleName` to tagged concrete `FunnelSpec`.
-}
makeFunnelDict : List (Handler m msg) -> (String -> m -> (Value -> Cmd msg)) -> FunnelDict m msg
makeFunnelDict hs portGetter =
  ( List.map handlerToFunnel hs |> Dict.fromList
  , portGetter
  )
...
-}
processValue : FunnelDict m msg -> Value -> State -> m -> Result String (m, Cmd msg)
processValue (funnelDict, portGetter) = PortFunnel.processValue funnelDict (appTrampoline portGetter)
